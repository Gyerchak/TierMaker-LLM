...
| `tiermaker-cover.png` | Cover photo (1920×960) for the template |

## Tiers (sorting order)

Sorting the `tiles/` folders alphabetically gives the tier order:
...
All logos are the property of their respective owners. `SOURCES-LOGOS.txt` lists the official source URL for every graphic used. Logos are included for identification/commentary in a ranking template under fair-use principles; replace with your own assets for commercial use.

Tier reports: Artificial Analysis Intelligence Index v4.2 (artificialanalysis.ai). Model availability: OpenCode workspace (opencode.ai).
